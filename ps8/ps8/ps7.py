...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        if (random.random() < (self.maxBirthProb * (1 - popDensity))):
            ...
            aVirus = SimpleVirus(self.maxBirthProb,self.clearProb)
            return aVirus
        else:
...
class SimplePatient(object):
    ...
    def update(self):
        ...
        for x in self.viruses:
            try:
                self.UpdatedViruses.append(x.reproduce(popDensity))
...
def simulationWithoutDrug():
    ...
    for i in range(numberOfViruses):
        ...
        i = SimpleVirus(0.1,0.05)
        viruses.append(i)

    firstPatient = SimplePatient(viruses,1000)
    timeSteps = []
    virusPop = []
    ...
    for x in range(300):
        ...
        virusPop.append(firstPatient.update())
        timeSteps.append(x)

    pylab.plot(timeSteps,virusPop)

    pylab.title("Growth of virus population with time")
    pylab.xlabel("Number of elasped time steps")
    pylab.ylabel("The population of the virus in the patient")
    pylab.show()
